// Logomark — official StraitsX symbol extracted from Figma.
// Paints with `color` if `currentColor` is passed; defaults to brand Vibrant Green.
// `tone="white"` (alias `variant="white"`/"mono") renders via currentColor so the
// mark inherits the parent text colour — ideal for white/mono on dark surfaces.
import React from "react";

export function Logomark({ size = 36, fill = "#00D37E", tone, variant, style, ...rest }) {
  const isMono = tone === "white" || tone === "mono" || variant === "white" || variant === "mono";
  const paint = isMono ? "currentColor" : fill;
  return (
    <svg
      width={size}
      height={size * (90 / 219)}
